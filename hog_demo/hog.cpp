...
//        void computeGradient(const GpuMat& img, GpuMat& grad, GpuMat& qangle, Stream& stream);
    ...
    void HOG_Impl::detect(InputArray _img, std::vector<Point>& hits, std::vector<double>* confidences)
    {
        ...
        if (confidences == NULL)
        {
            ...
            classify_hists(win_size_.height, win_size_.width,
        ...
        else
        {
            ...
            compute_confidence_hists(win_size_.height, win_size_.width,
                                          block_stride_.height, block_stride_.width,
                                          win_stride_.height, win_stride_.width,
                                          img.rows, img.cols,
                                          block_hists.ptr<float>(),
                           ...
                           Stream& stream)
    {
        ...
        case DESCR_FORMAT_ROW_BY_ROW:
            extract_descrs_by_rows(win_size_.height, win_size_.width,
                                        block_stride_.height, block_stride_.width,
                                        win_stride_.height, win_stride_.width,
                                        img.rows, img.cols,
                                        block_hists.ptr<float>(),
        ...
        case DESCR_FORMAT_COL_BY_COL:
            extract_descrs_by_cols(win_size_.height, win_size_.width,
                                        block_stride_.height, block_stride_.width,
                                        win_stride_.height, win_stride_.width,
                                        img.rows, img.cols,
                                        block_hists.ptr<float>(),
...
Ptr<cuda::HOG> cv::cuda::create(Size win_size,
                                     Size block_size,
                                     Size block_stride,
                                     Size cell_size,
                                     int nbins)
